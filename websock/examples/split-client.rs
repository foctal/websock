//! Echo client example demonstrating the split Sink/Stream API.
...
/// Command-line arguments for the split client.
#[derive(Parser, Debug)]
...
async fn main() -> anyhow::Result<()> {
    ...
    let conn = client.connect(url.as_str()).await?;

    // Split into Sink and Stream halves.
    let (mut sink, mut stream) = websock::stream::split(conn);

    sink.send(Message::Text("hello from split".into())).await?;

    let msg = stream.next().await.ok_or(websock_proto::Error::Closed)??;
